...
# 定义注意力机制模块, 自监督注意力机制，可以自适应地调整每个输入特征的重要程度，从而提高整个网络的表现力
...
class Net(nn.Module):
    ...
    def forward(self, noise_data, point_data, color_data, depth_data):
        ...
        concat_feature = torch.cat(  # 特征融合
            [noise_feature, point_feature, color_feature.unsqueeze(2), depth_feature.unsqueeze(2)], dim=2)

        concat_att = self.attention(concat_feature.permute(0, 2, 1))
        # x = concat_att.size()
        concat_feature = concat_feature + concat_att.unsqueeze(2)
...
#     x_pcd = torch.rand(1, 1036, 3).cuda()
#     x_point = torch.rand(1, 20, 3).cuda()
#     output = model(x_pcd, x_point, x_rgb, x_depth)
#     print(output.size())
