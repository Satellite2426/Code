...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            # test_loss.append(F.huber_loss(output, label, reduction='sum').item())  # sum up batch loss
            test_rmse.append(
                np.sqrt(mean_squared_error(label.cpu().numpy().flatten() * 100,
                                           output.detach().cpu().numpy().flatten() * 100)))
...
def main():
    ...
    if args.save_model:
        torch.save(model.state_dict(), f"model_pt/model_noise5000(SO-CNN-LSTM(output-less)_smooth(20))_{time_stamp}.pt")
